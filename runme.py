...
with open(input('Location of RFL_DB.dat:'),'rb') as f:
    f.read(4) # Read past the RNOD header
    mii_id = 1
    while True:
        data = f.read(74) # Grab 74 bytes
        if data.replace(b'\x00',b'') == b'':
            break # We are done.
        print(data)
        mii_name = Gen1Wii.from_bytes(data).mii_name.replace('\x00','')
        if mii_name.replace('?','').replace('!','') == '':
            mii_name = str(mii_id)
        mii = open(output_folder + '/{}.mii'.format(mii_name), 'wb')
        mii.write(data)
        mii.close()
        mii_id += 1
